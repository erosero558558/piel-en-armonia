...
const DEFAULT_OWNER_POLICY = 'detected_default_owner';
...
function defaultAcceptanceProfileForCodexInstance(codexInstance) {
    if (codexInstance === 'codex_frontend') {
        return 'frontend_delivery_checkpoint';
    }
    if (codexInstance === 'codex_transversal') {
        return 'transversal_runtime_checkpoint';
    }
    return 'backend_gate_checkpoint';
}

function resolveStrategyUpdatedAt(board) {
    return String(
...
function normalizeStrategySubfront(subfront) {
    ...
    const codexInstance = normalizeOptionalToken(subfront.codex_instance);
    return {
        ...
        default_acceptance_profile: String(
            subfront.default_acceptance_profile ||
                defaultAcceptanceProfileForCodexInstance(codexInstance)
        ).trim(),
...
function normalizeStrategyRecord(strategy) {
    ...
    return {
        ...
        owner_policy: String(
            strategy.owner_policy || DEFAULT_OWNER_POLICY
        ).trim(),
        ...
        for (const [bucket, scopes] of [
            ...
            for (const scope of scopes) {
                ...
                for (const owner of currentOwners) {
                    ...
                    if (
                        owner.bucket === 'support_only_scopes' &&
                        bucket === 'support_only_scopes'
                    ) {
                        continue;
                    }
                    errors.push(
...
function validateStrategyConfiguration(board, options = {}) {
    ...
    const safeUpdatedAt = resolveStrategyUpdatedAt(board);
...
function buildEmptyStrategySummary(board, activeStrategy, nextStrategy) {
    return {
        ...
        next: nextStrategy,
        updated_at: resolveStrategyUpdatedAt(board),
...
function buildStrategyCoverageSummary(board, options = {}) {
    ...
    summary.updated_at = resolveStrategyUpdatedAt(board);
    return summary;
}
