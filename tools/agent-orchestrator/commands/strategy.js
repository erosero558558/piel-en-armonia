...
function formatStrategySummary(summary = {}) {
    ...
    if (summary?.active) {
        ...
        lines.push(
            `Cobertura: aligned=${summary.aligned_tasks || 0}, support=${summary.support_tasks || 0}, exception=${summary.exception_tasks || 0}, orphan=${summary.orphan_tasks || 0}, dispersion=${summary.dispersion_score || 0}`
        );
    } else {
    ...
    if (overloaded.length > 0) {
        lines.push(
            `Subfrentes saturados: ${overloaded
                .map(
                    (row) =>
                        `${row.subfront_id}(${row.active_tasks}/${row.wip_limit})`
                )
                .join(', ')}`
        );
    }
