...
def find_method(name):
    # Find the public static function declaration WITHOUT catastrophic backtracking
    ...
    # Try to include the preceding docblock if exists
    doc_match = re.search(r'\/\*\*(?:(?!\/\*\*).)*?\*\/\s*$', text[:start_pos], re.DOTALL)
    if doc_match:
        doc_str = doc_match.group(0)
        # make sure doc_str isn't too huge (e.g. grabbing half the file)
        if len(doc_str) < 2000:
            start_pos -= len(doc_str)
