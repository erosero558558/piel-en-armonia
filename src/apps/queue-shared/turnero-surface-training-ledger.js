...
const STORAGE_KEY = 'turneroSurfaceTrainingLedgerV1';
const STORAGE_SCHEMA = 'turnero-surface-training-ledger/v1';
...
function normalizeEntry(entry = {}, fallbackSurfaceKey = 'operator') {
    ...
    const title =
        kind === 'manual_handoff'
            ? toString(source.title || source.label, 'Manual handoff evidence')
            : toString(source.title || source.label, 'Training readiness');
    ...
    return {
        ...
        state:
            toString(
                source.state,
                kind === 'manual_handoff' ? 'recorded' : 'recorded'
            ) || 'recorded',
        readiness: Number.isFinite(Number(source.readiness))
            ? Math.max(0, Math.min(100, Number(source.readiness)))
            : null,
...
function normalizeKind(value) {
    const normalized = toString(value, 'training').toLowerCase();
    if (
        normalized === 'manual_handoff' ||
        normalized === 'handoff' ||
        normalized === 'manual'
    ) {
...
export function createTurneroSurfaceTrainingLedger(
    scope = 'regional',
    clinicProfile = null
) {
    ...
    return {
        ...
        addTraining(entry = {}) {
            return this.add({
                ...entry,
                kind: 'training',
                title: entry.title || 'Training readiness',
                state: entry.state || 'recorded',
            });
        },
        addManualHandoff(entry = {}) {
            return this.add({
                ...entry,
                kind: 'manual_handoff',
                title: entry.title || 'Manual handoff evidence',
                state: entry.state || 'recorded',
            });
        },
        clear({ surfaceKey = '', kind = '' } = {}) {
            const normalizedSurfaceKey = toString(surfaceKey);
            const normalizedKind = toString(kind).toLowerCase();
            if (!normalizedSurfaceKey && !normalizedKind) {
        ...
        summary({ surfaceKey = '' } = {}) {
            ...
            return {
                total: list.length,
                training: list.filter((entry) => entry.kind === 'training')
                    .length,
                manualHandoff: list.filter(
...
export { STORAGE_KEY as TURNERO_SURFACE_TRAINING_LEDGER_STORAGE_KEY };
