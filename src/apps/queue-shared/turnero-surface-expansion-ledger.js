...
import {
    ...
    resolveTurneroSurfaceExpansionScope,
} from './turnero-surface-expansion-snapshot.js';

const STORAGE_KEY = 'turneroSurfaceExpansionLedgerV1';
const STORAGE_SCHEMA = 'turnero-surface-expansion-ledger/v1';
...
function normalizeScope(scope, clinicProfile) {
    return resolveTurneroSurfaceExpansionScope(scope, clinicProfile);
}

function normalizeKind(value) {
    const normalized = toString(value, 'module-hint')
        .toLowerCase()
        .replace(/[\s_]+/g, '-');
    ...
    if (['module', 'addon', 'next-module'].includes(normalized)) {
        return 'module-hint';
    }
    if (['note', 'followup', 'follow-up'].includes(normalized)) {
        return 'followup-note';
    }
    if (['discovery', 'discovery-item'].includes(normalized)) {
        return 'discovery-note';
    }

    return normalized || 'module-hint';
}
...
function normalizeStatus(value) {
    ...
    if (['watch', 'review', 'pending', 'queued', 'active'].includes(normalized)) {
        return 'watch';
    }
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        id:
            toString(source.id) ||
            `expansion-ledger-${Date.now()}-${Math.random()
                .toString(36)
                .slice(2, 8)}`,
        scope,
        surfaceKey: normalizeTurneroSurfaceExpansionSurfaceKey(
            source.surfaceKey || source.surface || 'surface'
        ),
        kind,
        status: normalizeStatus(
            source.status || (kind === 'followup-note' ? 'watch' : 'ready')
        ),
        title: toString(source.title || source.label, 'Expansion item'),
...
export function createTurneroSurfaceExpansionLedger(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = normalizeScope(scope, clinicProfile);
    ...
    return {
        list({ surfaceKey = '', kind = '', status = '' } = {}) {
            const normalizedSurfaceKey = toString(surfaceKey)
                ? normalizeTurneroSurfaceExpansionSurfaceKey(surfaceKey)
                : '';
