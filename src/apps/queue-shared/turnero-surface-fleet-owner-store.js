...
const STORAGE_KEY = 'turneroSurfaceFleetOwnerStoreV1';
const STORE_SCHEMA = 'turnero-surface-fleet-owner-store/v1';
...
function normalizeStatus(value) {
    ...
    if (['paused', 'hold', 'suspended', 'pending'].includes(normalized)) {
        return 'paused';
    }
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        id:
            toString(source.id) ||
            `fleet-owner-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        scope,
        region: scope,
        surfaceKey: normalizeSurfaceKey(source.surfaceKey || source.surface),
        actor,
        owner: actor,
        role: toString(source.role || 'regional', 'regional'),
...
export function createTurneroSurfaceFleetOwnerStore(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = normalizeScope(scope, clinicProfile);
    ...
    return {
        ...
        add(entry = {}) {
            const actor = toString(
                entry.actor || entry.owner || entry.name || 'owner',
                'owner'
            );
