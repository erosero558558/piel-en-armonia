...
const STORAGE_KEY = 'turneroSurfaceRenewalOwnerStoreV1';
const STORAGE_SCHEMA = 'turnero-surface-renewal-owner-store/v1';
...
function normalizeRole(value) {
    ...
    if (['renewal', 'commercial', 'success'].includes(normalized)) {
        return normalized;
    }
    return 'renewal';
}
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        id:
            toString(source.id) ||
            `renewal-owner-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function createTurneroSurfaceRenewalOwnerStore(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = toString(scope, 'regional') || 'regional';
