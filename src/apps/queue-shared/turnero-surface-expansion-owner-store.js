...
const STORAGE_KEY = 'turneroSurfaceExpansionOwnerStoreV1';
const STORAGE_SCHEMA = 'turnero-surface-expansion-owner-store/v1';
...
function normalizeEntry(entry = {}, fallbackScope = 'regional') {
    ...
    return {
        id:
            toString(source.id) ||
            `expansion-owner-${Date.now()}-${Math.random()
                .toString(36)
                .slice(2, 8)}`,
        scope,
        ...
        actor,
        owner: actor,
        role: toString(source.role || 'expansion', 'expansion'),
...
export function createTurneroSurfaceExpansionOwnerStore(
    scope = 'regional',
    clinicProfile = null
) {
    const normalizedScope = normalizeScope(scope, clinicProfile);
