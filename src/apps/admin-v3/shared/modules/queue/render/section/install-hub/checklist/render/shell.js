...
import { formatOpeningChecklistDate } from '../../state.js';

function renderOpeningChecklistStep(step, checklist, assist) {
    const isReady = Boolean(checklist.steps[step.id]);
    const isSuggested =
        !isReady && Boolean(assist.suggestions[step.id]?.suggested);
...
export function renderOpeningChecklistShell(steps, checklist, assist) {
    ...
    return `
        <section class="queue-opening-checklist__shell">
            ...
            <div id="queueOpeningChecklistSteps" class="queue-opening-checklist__steps" role="list" aria-label="Checklist de apertura diaria">
                ${steps
                    .map((step) =>
                        renderOpeningChecklistStep(step, checklist, assist)
                    )
                    .join('')}
            </div>
        </section>
