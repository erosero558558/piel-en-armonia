...
const ROOT = cli.root
    ...
    : resolve(__dirname, '..');
const PUBLIC_ENTRY = resolve(ROOT, 'script.js');
const PUBLIC_CHUNKS_DIR = resolve(ROOT, 'js', 'chunks');
...
function collectReachableChunks() {
    ...
    const pendingFiles = [PUBLIC_ENTRY];
    ...
    while (pendingFiles.length > 0) {
        ...
        for (const specifier of specifiers) {
            ...
            const nextChunkPath = resolve(PUBLIC_CHUNKS_DIR, chunkFilename);
...
function collectActivePublicAssets(reachableChunks) {
    return [
        PUBLIC_ENTRY,
        ...Array.from(reachableChunks, (chunkFilename) =>
            resolve(PUBLIC_CHUNKS_DIR, chunkFilename)
        ),
...
function main() {
    ...
    if (!existsSync(PUBLIC_CHUNKS_DIR)) {
        warn(
    ...
    const filesInDirectory = readdirSync(PUBLIC_CHUNKS_DIR).filter((file) =>
        file.endsWith('.js')
    );
    ...
    const mergeConflictFindings = collectActivePublicAssets(reachableChunks)
    ...
    if (staleChunks.length === 0) {
        log(
            `[public-chunks] Limpieza OK. Chunks vigentes: ${reachableChunks.size}.`
        );
        process.exit(0);
    }
    ...
    for (const chunk of staleChunks) {
        const targetPath = resolve(PUBLIC_CHUNKS_DIR, chunk);
        try {
    ...
    log(
        `[public-chunks] Limpieza completada. Eliminados ${deleted}, vigentes ${reachableChunks.size}.`
    );
}
