...
function countLines(file) {
  try { return read(resolve(ROOT, file)).split('\n').length; }
  catch { return 0; }
}
...
const checks = {
  ...
  'S1-04': () => {
    try {
      ...
      return m.name.includes('Aurora Derm') && !m.name.includes('Flow OS');
    } catch { return false; }
  },
  ...
  'S2-01': () => {
    const idx = read(resolve(ROOT, 'index.html'));
    return idx.includes('"@type":"Dermatology"') || idx.includes('"@type":"MedicalClinic"');
  },

  'S2-03': () => {
    const idx = read(resolve(ROOT, 'index.html'));
    return idx.includes('og:title') && idx.includes('og:description') && idx.includes('og:image');
  },
  ...
  'S2-11': () => fileExists('es/blog/como-elegir-dermatologo-quito/index.html'),
  ...
  'S4-21': () => {
    try {
      const count = parseInt(
        execSync(`find "${resolve(ROOT, 'src/apps/queue-shared')}" -name "*.js" | wc -l`, { encoding: 'utf8' }).trim()
      );
...
function main() {
  ...
  const results = { alreadyDone: [], nowDone: [], stillPending: [], unchecked: [] };

  for (const [taskId, check] of Object.entries(checks)) {
    const taskInfo = taskLines[taskId];
    if (!taskInfo) { results.unchecked.push(taskId); continue; }
  ...
  if (results.nowDone.length > 0) {
    ...
    if (FIX) {
      ...
      console.log(`   Run: git add AGENTS.md && git commit -m "docs: sync board with actual state"`);
    } else {
