...
require_once __DIR__ . '/telemedicine/TelemedicineRepository.php';
...
final class CaseMediaFlowService
{
    ...
    private static function isPublicPipelineUpload(array $upload): bool
    {
        ...
        if ($kind === self::KIND_PAYMENT_PROOF) {
            return false;
        }
        if ($kind === self::KIND_SUPPORTING_DOCUMENT) {
            return false;
        }
    ...
    private static function classifyAssetKind(string $filename, string $fallbackKind): string
    {
        ...
        if (str_contains($normalized, 'document') || str_contains($normalized, 'doc') || $fallbackKind === self::KIND_SUPPORTING_DOCUMENT) {
            return 'document';
        }
        return 'progress';
    }
    ...
    // --- Ported from ClinicalMediaService ---
