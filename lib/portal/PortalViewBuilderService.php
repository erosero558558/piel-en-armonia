...
final class PortalViewBuilderService
{
    ...
    function buildPortalHistory(array $store, array $snapshot, array $patient, ?string $tenantId = null): array
    {
        ...
        foreach (($store['appointments'] ?? []) as $appointment) {
            ...
            if (!is_array($appointment) || !PortalHistoryService::appointmentMatchesPatient($appointment, $snapshot)) {
                continue;
            }
            ...
            $caseId = PortalHistoryService::resolveAppointmentCaseId($appointment, $snapshot);
            ...
            if (!PortalHistoryService::shouldIncludeConsultationInHistory($status, $timestamp, $documents)) {
                continue;
            }
        ...
        foreach ($caseIds as $caseId) {
            ...
            $documents = $documentsByCase[$caseId] ?? PatientPortalDocumentController::defaultDocumentState($caseId);
            if (!PortalHistoryService::documentsHavePortalSignal($documents)) {
                continue;
            }

            $caseRecord = PortalHistoryService::findPatientCaseRecord($store, $caseId, $tenantId);
        ...
        $upcomingAppointment = PortalHistoryService::findNextAppointment($store, $snapshot, $tenantId);
        ...
        foreach (array_keys($caseMap) as $caseId) {
            ...
            $documentsByCase[$caseId] = [
                'prescription' => PatientPortalDocumentController::buildPortalDocumentPayload(
                    ...
                    PortalHistoryService::hasPendingPrescriptionDraft($drafts)
                ),
