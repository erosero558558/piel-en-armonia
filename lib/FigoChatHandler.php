...
class FigoChatHandler
{
    /**
     * Entry point for the chat handler.
     */
    public function handle(): void
    {
        $this->figo_apply_cors();
        ...
        $endpointDiagnostics = $this->figo_endpoint_diagnostics($endpoint);
        $recursiveConfigDetected = $this->figo_is_recursive_endpoint($endpoint);
        ...
        if ($requestMethod === 'GET') {
            if ($providerMode === 'openclaw_queue') {
                ...
                $upstreamReachable = $this->figo_probe_upstream($endpoint, 3);
            }
        }
        ...
        if ($method === 'GET') {
            audit_log_event('figo.status', [
                ...
                'degradedMode' => $this->figo_degraded_mode_enabled(),
        ...
        $lastUserMessage = $this->figo_last_user_message($messages);
        $fastLocalContent = $this->figo_fast_local_content($this->figo_normalize_text($lastUserMessage));
        if ($fastLocalContent !== '') {
            try {
            ...
            $fastPayload = $this->figo_finalize_completion([
                'id' => $id,
            ...
            $this->figo_json_post_response($fastPayload, 200, $postRequestStartedAt, $providerMode);
        }
        ...
        if ($providerMode === 'openclaw_queue') {
            ...
            if (($bridgePayload['mode'] ?? '') === 'queued') {
                ...
                $this->figo_json_post_response($bridgePayload, 200, $postRequestStartedAt, $providerMode);
            }
        ...
        if ($endpoint === '') {
            ...
            $fallback = $this->figo_finalize_completion(
                $this->figo_build_fallback_completion($model, $messages),
                'degraded',
                'fallback',
                'endpoint_missing',
                false,
            ...
            $this->figo_json_post_response($fallback, 200, $postRequestStartedAt, $providerMode);
        }
        ...
        if ($recursiveConfigDetected) {
            ...
            if ($this->figo_degraded_mode_enabled()) {
                $fallback = $this->figo_finalize_completion(
                    $this->figo_build_fallback_completion($model, $messages),
                    'degraded',
                    'fallback',
        ...
        $failfast = $this->figo_check_failfast_window();
        if (($failfast['active'] ?? false) === true) {
            ...
            $this->sendHeader('Retry-After: ' . max(1, $retryAfter));
        ...
        if (!is_string($rawResponse)) {
            $this->figo_write_upstream_state(false, 'curl_failed');
        ...
        if ($httpCode >= 400) {
            $this->figo_write_upstream_state(false, 'http_' . $httpCode);
        ...
        $content = $this->figo_extract_text($decoded, $rawResponse);
        if ($this->figo_is_upstream_technical_fallback($content)) {
            audit_log_event('figo.fallback', [
        ...
        $this->figo_write_upstream_state(true, '');
        ...
        $successPayload = $this->figo_finalize_completion([
            'id' => $id,
            'object' => 'chat.completion',
            'created' => time(),
            'model' => $model,
        ...
        $this->figo_json_post_response($successPayload, 200, $postRequestStartedAt, $providerMode);
    }
    ...
    protected function figo_apply_cors(): void
    {
        ...
        foreach ($allowed as $allowedOrigin) {
            if (strcasecmp($normalizedOrigin, $allowedOrigin) === 0) {
                ...
                $this->sendHeader('Cross-Origin-Resource-Policy: cross-origin');
                return;
            }
        }
    }

    protected function figo_extract_text(array $decoded, string $raw): string
    {
    ...
    protected function figo_is_upstream_technical_fallback(string $content): bool
    {
    ...
    protected function figo_normalize_text(string $text): string
    {
    ...
    protected function figo_last_user_message(array $messages): string
    {
    ...
    protected function figo_contains_any_pattern(string $text, array $patterns): bool
    {
    ...
    protected function figo_fast_local_content(string $normalizedText): string
    {
        if ($normalizedText === '') {
        ...
        if ($this->figo_contains_any_pattern($normalizedText, [
            '/\bping\b/',
            '/\blatencia\b/',
            '/\btest\b/',
            '/\bprueba\b/'
    ...
    protected function figo_endpoint_diagnostics(string $endpoint): array
    {
        if ($endpoint === '') {
            return [
                'configured' => false,
    ...
    protected function figo_is_recursive_endpoint(string $endpoint): bool
    {
    ...
    protected function figo_probe_upstream(string $endpoint, int $timeoutSeconds = 3): ?bool
    {
        if (trim($endpoint) === '') {
            return null;
        }
        ...
        if ($this->figo_is_recursive_endpoint($endpoint)) {
            return false;
        }
    ...
    protected function figo_finalize_completion(
        array $response,
        string $mode,
        string $source,
        string $reason = '',
    ...
    protected function figo_build_fallback_completion(string $model, array $messages): array
    {
    ...
    protected function figo_degraded_mode_enabled(): bool
    {
    ...
    protected function figo_failfast_window_seconds(): int
    {
    ...
    protected function figo_upstream_state_path(): string
    {
    ...
    protected function figo_read_upstream_state(): array
    {
        $path = $this->figo_upstream_state_path();
        if (!is_file($path)) {
            return [];
        }
    ...
    protected function figo_write_upstream_state(bool $up, string $reason = ''): void
    {
        $payload = [
            'up' => $up,
            'reason' => trim($reason),
        ...
        @file_put_contents(
            $this->figo_upstream_state_path(),
            (string) json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
        );
    }

    protected function figo_check_failfast_window(): array
    {
        $state = $this->figo_read_upstream_state();
        ...
        $window = $this->figo_failfast_window_seconds();
    ...
    protected function figo_metric_label(string $value, string $fallback): string
    {
    ...
    protected function figo_record_post_metrics(float $startedAt, string $providerMode, array $payload, int $statusCode): void
    {
        ...
        $mode = $this->figo_metric_label((string) ($payload['mode'] ?? ''), $statusCode >= 400 ? 'degraded' : 'live');
        $source = $this->figo_metric_label((string) ($payload['source'] ?? ''), 'none');
        $reason = $this->figo_metric_label((string) ($payload['reason'] ?? ''), $statusCode >= 400 ? ('http_' . $statusCode) : 'none');
        $provider = $this->figo_metric_label($providerMode, 'legacy_proxy');
    ...
    protected function figo_json_post_response(array $payload, int $statusCode, float $startedAt, string $providerMode): void
    {
        $this->figo_record_post_metrics($startedAt, $providerMode, $payload, $statusCode);
