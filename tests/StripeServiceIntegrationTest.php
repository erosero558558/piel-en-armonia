    ...
    class PaymentIntent
    {
        ...
        public function __construct($data)
        {
            ...
            if (!isset($this->data['id'])) {
                $this->data['id'] = 'pi_mock_123';
            }
            if (!isset($this->data['client_secret'])) {
                $this->data['client_secret'] = 'pi_mock_secret_123';
            }
            if (!isset($this->data['amount'])) {
                $this->data['amount'] = 4000;
            }
...
namespace {
    ...
    run_test('payment_build_idempotency_key format', function () {
        ...
        if (strpos($key, 'test-') !== 0) {
            throw new Exception("Key should start with test-");
        }
        if (strlen($key) !== 53) {
            throw new Exception("Key length should be 53");
        }
    });
    ...
    run_test('stripe_create_payment_intent success', function () {
        ...
        if (!isset($intent['id'])) {
            throw new Exception("Intent ID missing");
        }
