...
test.describe('Mobile overflow regressions', () => {
  test('critical sections stay inside viewport on mobile width', async ({ page }) => {
    ...
    for (const sectionId of SECTION_IDS) {
      ...
      expect(metrics.offenders, `Overflow in section #${sectionId}`).toEqual([]);
    }
  });

  test('chat container stays fully visible when opened on mobile', async ({ page }) => {
    await page.setViewportSize({ width: 360, height: 800 });
    await page.goto('/');
    ...
    expect(chatRect.x + chatRect.width).toBeLessThanOrEqual(viewport.width + 1);
    expect(chatRect.y + chatRect.height).toBeLessThanOrEqual(viewport.height + 1);
  });
});
