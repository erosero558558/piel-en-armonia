...
console.assert(tax2 === 22.5, '❌ IVA 15% de $150 debería ser $22.50');
...
const total1 = computeTotal(40, 0.0);
...
console.assert(total2 === 172.5, '❌ Total con IVA 15% debería ser $172.50');
...
const total4 = computeTotal(30, 0.0);
...
console.assert(roundToTwo(22.504) === 22.5, '❌ Redondeo incorrecto');
...
const formatted2 = formatMoney(172.5, { showCurrency: false });
...
console.assert(
    formatted4 === '$0.00',
    `❌ Formato de inválido incorrecto: ${formatted4}`
);
...
console.assert(serviceConsulta.taxRate === 0.0, '❌ Tax rate debería ser 0');
console.log(
    `✅ Servicio consulta: ${serviceConsulta.name} - $${serviceConsulta.priceBase} (IVA: ${serviceConsulta.taxRate}%)`
);
...
console.log(
    `✅ Servicio láser: ${serviceLaser.name} - Desde $${serviceLaser.priceBase} (IVA: ${serviceLaser.taxRate}%)`
);

const serviceNotFound = getServiceById('no-existe');
console.assert(
    serviceNotFound === null,
    '❌ Servicio inexistente debería retornar null'
);
...
console.assert(
    priceInfo1.formatted.total === '$40.00',
    `❌ Formato incorrecto: ${priceInfo1.formatted.total}`
);
console.log(
    `✅ PriceInfo consulta: ${priceInfo1.formatted.display} (IVA: ${priceInfo1.taxLabel})`
);

// Caso 2: Láser con IVA 15%
const priceInfo2 = getServicePriceInfo('laser');
console.assert(
    priceInfo2.total === 172.5,
    `❌ Total debería ser $172.50, es $${priceInfo2.total}`
);
console.assert(
    priceInfo2.taxAmount === 22.5,
    `❌ Tax debería ser $22.50, es $${priceInfo2.taxAmount}`
);
console.log(
    `✅ PriceInfo láser: ${priceInfo2.formatted.display} (IVA: ${priceInfo2.taxLabel})`
);
...
console.assert(
    breakdown1.formatted.total === '$40.00',
    '❌ Formato total incorrecto'
);
...
console.assert(
    breakdown2.pricing.total < 40,
    '❌ Total con descuento debería ser menor'
);
...
console.assert(
    valid2 === true,
    '❌ Monto dentro de tolerancia debería ser válido'
);
...
const allHealthZeroTax = healthServices.every((s) => s.taxRate === 0.0);
console.assert(
    allHealthZeroTax,
    '❌ Todos los servicios clínicos deben tener IVA 0%'
);
console.log(
    `✅ Servicios clínicos con IVA 0%: ${healthServices.length} servicios`
);

// Regla: Servicios estéticos tienen IVA 15%
const estheticServices = SERVICES_CONFIG.filter(
    (s) => s.category === 'estetico'
);
const allEstheticHaveTax = estheticServices.every(
    (s) => s.taxRate === IVA_GENERAL_RATE
);
console.assert(
    allEstheticHaveTax,
    '❌ Todos los servicios estéticos deben tener IVA 15%'
);
console.log(
    `✅ Servicios estéticos con IVA 15%: ${estheticServices.length} servicios`
);

// Regla: IVA general rate es 15%
console.assert(IVA_GENERAL_RATE === 0.15, '❌ IVA general debe ser 15%');
console.log(
    `✅ IVA general configurado: ${(IVA_GENERAL_RATE * 100).toFixed(0)}%`
);
