...
test.describe('Deferred stylesheet version parity', () => {
  for (const path of PAGE_PATHS) {
    test(`page ${path} references the current deferred stylesheet version`, async ({ page }) => {
      ...
      const locator = page.locator('link[rel="preload"][as="style"][href*="styles-deferred.css"]').first();
      await expect(locator).toHaveAttribute('href', new RegExp(EXPECTED_DEFERRED_VERSION));
