...
- [x] **S15-05** `[M]` Alertas de claim abandonado — ✅ _Corregido hoy mismo._ `bin/claim.js` ahora expone `list` como alias de `status` y muestra la sección `⚠️ Claims expiradas:` con tarea y agente cuando detecta claims vencidas. `agent-orchestrator status --json` también publica `expiryWarning` para dashboards. Verificable: un claim con fecha pasada aparece en el output de `node bin/claim.js list`.
