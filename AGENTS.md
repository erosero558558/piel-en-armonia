...
- [x] **S15-06** `[S]` Detector de commits done sin claim — ✅ _Corregido hoy mismo._ `sync-backlog.js` detecta tareas que acaban de pasar de `[ ]` a `[x]` vía diff en `AGENTS.md` y emite warning si no encuentra evidencia de claim en el cambio actual. También se añadió el alias `npm run sync:backlog` para validar ese flujo tal cual. Verificable: `npm run sync:backlog` → advierte si hay done sin claim.
