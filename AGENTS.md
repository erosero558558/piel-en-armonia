...
- [x] **S15-08** `[M]` `bin/health-check.js` — ✅ _Corregido hoy mismo._ Se añadió un doctor rápido del sistema de agentes con checks para regex canónica, sync de backlog, cobertura de sitemap, deuda de scripts rotos y convergencia entre `report` y `agent-orchestrator`. También quedó expuesto como `npm run agent:health`. Verificable: `npm run agent:health` devuelve semáforo por check; en un repo sano termina en exit 0.
...
- [x] **S15-11** `[M]` `bin/dispatch.js` — ✅ _Corregido hoy mismo._ `dispatch` ahora aplica WIP limit por rol/lane, configurable por rol con defaults alineados a S14 y override por CLI/env. Si el lane ya alcanzó su límite de claims activas, el comando no recomienda otra tarea y devuelve el mensaje `WIP limit reached — termina una tarea antes`, listando los claims activos que están saturando ese rol. También quedó con contrato Node y smoke CLI sobre repo temporal para el caso bloqueado y el caso con override. Verificable: `node bin/dispatch.js --role backend` → si hay 2 claims activas backend, muestra el bloqueo de WIP.
