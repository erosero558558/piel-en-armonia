...
- [x] **S15-09** `[M]` `bin/regression-watch.js` — ✅ _Corregido hoy mismo._ Se añadió un watchdog que cruza tareas `[x]` con archivos explícitos de `AGENTS.md` contra `git diff --stat`/`--name-status` de la ventana reciente para alertar solo cuando un archivo mencionado quedó borrado o en 0 bytes. Expone salida texto/JSON y deja contrato Node con repos temporales para el caso sano y el caso con borrado. Verificable: `node bin/regression-watch.js` → lista regresiones sospechosas o `✅ 0 regresiones detectadas`.
- [x] **S15-10** `[S]` `bin/report.js` — ✅ _Corregido hoy mismo._ El reporte diario ahora incorpora la sección `Regresiones sospechosas`, conectada directamente con `regression-watch.js`, y la renderiza tanto en consola como en Markdown sin volver frágil el comando cuando detecta hallazgos. También quedó cubierta con una prueba CLI en repos temporales para el caso con regresión y el caso sano. Verificable: `npm run report --silent` → sin error aunque `regression-watch` detecte issues.
