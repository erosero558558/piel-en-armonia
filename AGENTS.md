...
- [x] **S15-12** `[S]` `README.md` — ✅ _Corregido hoy mismo._ `README.md` ahora tiene un bloque parcial generado entre `<!-- STATS_START -->` y `<!-- STATS_END -->`, rellenado por `bin/gen-readme-stats.js` con done/total/pct y sprint activo leídos de `AGENTS.md`. Además, `bin/sync-backlog.js` invoca ese generador para mantener el `README` sincronizado cada vez que se actualiza el backlog. Verificable: `node bin/gen-readme-stats.js && grep "Progreso real del board" README.md` → match con los valores actuales.
