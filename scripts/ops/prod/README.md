...
- `CHECKLIST-HOST-PUBLIC-SYNC.ps1` imprime un checklist host-side reutilizable para comparar `/root/sync-pielarmonia.sh` contra el wrapper canonico, capturar `__hosting/runtime`, `release-target.runtime.json`, `main-sync-status.json`, revisar `health-diagnostics` y validar `storeEncryptionCompliant`.
