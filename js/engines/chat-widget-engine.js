!(function () {
    'use strict';
    let t = null,
        e = !1,
        o = !1;
    const n = {
        services: 'Que servicios ofrecen?',
        prices: 'Cuales son los precios?',
        telemedicine: 'Como funciona la consulta online?',
        human: 'Quiero hablar con un doctor real',
        acne: 'Tengo problemas de acne',
        laser: 'Informacion sobre tratamientos laser',
        location: 'Donde estan ubicados?',
    };
    function i() {
        return (
            !(!t || 'function' != typeof t.getChatbotOpen) &&
            !0 === t.getChatbotOpen()
        );
    }
    function a(e) {
        t &&
            'function' == typeof t.setChatbotOpen &&
            t.setChatbotOpen(!0 === e);
    }
    function s() {
        return (
            (t &&
                'function' == typeof t.getChatHistoryLength &&
                Number(t.getChatHistoryLength())) ||
            0
        );
    }
    function c() {
        if (t && 'function' == typeof t.getClinicalHistorySession) {
            const e = t.getClinicalHistorySession();
            return e && 'object' == typeof e ? e : null;
        }
        return null;
    }
    function r() {
        t && 'function' == typeof t.renderChatHistory && t.renderChatHistory();
    }
    function u(e) {
        return t && 'function' == typeof t.ensureClinicalSessionHydrated
            ? t.ensureClinicalSessionHydrated(e || {})
            : Promise.resolve(null);
    }
    function l() {
        t && 'function' == typeof t.scrollToBottom && t.scrollToBottom();
    }
    function d(o) {
        var n;
        e ||
            ((e = !0),
            (n = { source: o || 'widget' }),
            t &&
                'function' == typeof t.trackEvent &&
                t.trackEvent('chat_started', n || {}));
    }
    function f(e) {
        t && 'function' == typeof t.addBotMessage && t.addBotMessage(e);
    }
    function g(e) {
        if (t && 'function' == typeof t.addUserMessage)
            return t.addUserMessage(e);
    }
    function m(e) {
        return t && 'function' == typeof t.processWithKimi
            ? t.processWithKimi(e)
            : Promise.resolve();
    }
    function h() {
        return 'function' == typeof window.matchMedia
            ? window.matchMedia('(max-width: 900px)').matches
            : window.innerWidth <= 900;
    }
    function p() {
        const e = document.documentElement,
            o = document.body;
        if (!e || !o) return;
        const n = (function () {
                const t = document.querySelector('.quick-dock');
                if (!t || 'function' != typeof t.getBoundingClientRect)
                    return 0;
                const e = window.getComputedStyle
                    ? window.getComputedStyle(t)
                    : null;
                if (e && ('none' === e.display || 'hidden' === e.visibility))
                    return 0;
                const o = t.getBoundingClientRect();
                return Number.isFinite(o.height)
                    ? Math.ceil(Math.max(o.height, 0))
                    : 0;
            })(),
            a = Math.max(0, n - 56),
            s = window.innerWidth <= 640 ? 76 : 82;
        (e.style.setProperty('--chat-widget-mobile-bottom', `${s + a}px`),
            e.style.setProperty(
                '--chat-container-mobile-bottom',
                `${146 + a}px`
            ),
            e.style.setProperty('--chat-quick-dock-height', `${n}px`),
            h()
                ? e.style.setProperty(
                      '--chat-mobile-top-offset',
                      `${
                          (function () {
                              let t = 0;
                              return (
                                  ['.language-bar', '.nav'].forEach((e) => {
                                      const o = document.querySelector(e);
                                      if (!o) return;
                                      const n = o.getBoundingClientRect();
                                      Number.isFinite(n.bottom) &&
                                          (t = Math.max(t, n.bottom));
                                  }),
                                  Math.ceil(Math.max(t, 96))
                              );
                          })() + 10
                      }px`
                  )
                : e.style.removeProperty('--chat-mobile-top-offset'),
            o.classList.toggle('chatbot-open', i()),
            o.classList.toggle(
                'chatbot-booking-active',
                i() &&
                    !(!t || 'function' != typeof t.isChatBookingActive) &&
                    !0 === t.isChatBookingActive()
            ),
            o.classList.toggle('chatbot-mobile-open', h() && i()));
    }
    function b() {
        return (
            'true' === localStorage.getItem('forceAI') ||
            'file:' !== window.location.protocol
        );
    }
    function y() {
        return (
            'clinical_intake' ===
            ((t &&
                'function' == typeof t.getChatMode &&
                String(t.getChatMode() || '').trim()) ||
                'general')
        );
    }
    function v(t, e) {
        if (y()) {
            const t = (function (t) {
                if (!t || 'object' != typeof t) return null;
                const e =
                        t.metadata && 'object' == typeof t.metadata
                            ? t.metadata
                            : null,
                    o =
                        e && e.pendingPatientAction
                            ? e.pendingPatientAction
                            : null;
                return o && 'object' == typeof o ? o : null;
            })(e);
            let o =
                'Hola. Vamos a continuar tu <strong>historia clinica</strong> en este mismo chat.<br><br>';
            return t &&
                'string' == typeof t.question &&
                '' !== t.question.trim()
                ? ((o +=
                      'Tu medico dejo una pregunta pendiente para seguir con el caso:<br><br>'),
                  (o += `<strong>${String(t.question).trim()}</strong><br><br>`),
                  (o +=
                      'Responde con tus propias palabras en el cuadro de abajo y seguimos desde ahi.'),
                  o)
                : ((o +=
                      'Escribe con tus palabras que te esta pasando y te hare una pregunta a la vez. '),
                  (o +=
                      'No necesitas llenar formularios largos ni explicar todo de golpe.'),
                  o);
        }
        let o =
            'Hola! Soy el <strong>Dr. Virtual</strong> de <strong>Aurora Derm</strong>.<br><br>';
        return (
            t
                ? ((o +=
                      '<strong>Conectado con Inteligencia Artificial</strong><br><br>'),
                  (o += 'Puedo ayudarte con informacion detallada sobre:<br>'),
                  (o += '- Nuestros servicios dermatologicos<br>'),
                  (o += '- Precios de consultas y tratamientos<br>'),
                  (o += '- Agendar citas presenciales o online<br>'),
                  (o += '- Ubicacion y horarios de atencion<br>'),
                  (o +=
                      '- Resolver tus dudas sobre cuidado de la piel<br><br>'))
                : ((o += 'Puedo ayudarte con informacion sobre:<br>'),
                  (o += '- Nuestros servicios dermatologicos<br>'),
                  (o += '- Precios de consultas y tratamientos<br>'),
                  (o += '- Agendar citas presenciales o online<br>'),
                  (o += '- Ubicacion y horarios de atencion<br><br>')),
            (o += 'En que puedo ayudarte hoy?'),
            o
        );
    }
    function w() {
        const e = b(),
            o = c();
        (!(function (...e) {
            t && 'function' == typeof t.debugLog && t.debugLog(...e);
        })(
            'Estado del chatbot:',
            y() ? 'HISTORIA CLINICA' : e ? 'IA REAL' : 'Respuestas locales'
        ),
            f(v(e, o)),
            y() ||
                setTimeout(() => {
                    let t = '<div class="chat-suggestions">';
                    ((t +=
                        '<button class="chat-suggestion-btn" data-action="quick-message" data-value="services">'),
                        (t +=
                            '<i class="fas fa-stethoscope"></i> Ver servicios'),
                        (t += '</button>'),
                        (t +=
                            '<button class="chat-suggestion-btn" data-action="quick-message" data-value="appointment">'),
                        (t +=
                            '<i class="fas fa-calendar-check"></i> Agendar cita'),
                        (t += '</button>'),
                        (t +=
                            '<button class="chat-suggestion-btn" data-action="quick-message" data-value="prices">'),
                        (t += '<i class="fas fa-tag"></i> Consultar precios'),
                        (t += '</button>'),
                        (t += '</div>'),
                        f(
                            '<div class="chat-suggestions"><button class="chat-suggestion-btn" data-action="quick-message" data-value="services"><i class="fas fa-stethoscope"></i> Ver servicios</button><button class="chat-suggestion-btn" data-action="quick-message" data-value="appointment"><i class="fas fa-calendar-check"></i> Agendar cita</button><button class="chat-suggestion-btn" data-action="quick-message" data-value="prices"><i class="fas fa-tag"></i> Consultar precios</button></div>'
                        ));
                }, 500));
    }
    async function C() {
        const t = document.getElementById('chatInput');
        if (!t) return;
        const e = String(t.value || '').trim();
        e &&
            (d('first_message'),
            await Promise.resolve(g(e)).catch(() => {}),
            (t.value = ''),
            await Promise.resolve(m(e)).catch(() => {}));
    }
    function P() {
        const t = document.getElementById('chatTeaser');
        t && t.classList.add('show');
    }
    function k() {
        const t = document.getElementById('chatTeaser');
        t && t.classList.remove('show');
    }
    ((window.Piel = window.Piel || {}),
        (window.Piel.ChatWidgetEngine = {
            init: function (e) {
                return (
                    (t = e || {}),
                    (function () {
                        if (o) return;
                        o = !0;
                        const t = () => {
                            p();
                        };
                        (window.addEventListener('resize', t, { passive: !0 }),
                            window.addEventListener('orientationchange', t, {
                                passive: !0,
                            }),
                            window.visualViewport &&
                                window.visualViewport.addEventListener(
                                    'resize',
                                    t,
                                    { passive: !0 }
                                ));
                    })(),
                    p(),
                    window.Piel && window.Piel.ChatWidgetEngine
                );
            },
            toggleChatbot: function () {
                const e = document.getElementById('chatbotContainer');
                if (!e) return;
                t && 'function' == typeof t.warmChatUi && t.warmChatUi();
                const o = !i();
                if ((a(o), !o)) return (e.classList.remove('active'), void p());
                (p(), e.classList.add('active'), k());
                const n = document.getElementById('chatNotification');
                if (
                    (n && (n.style.display = 'none'),
                    l(),
                    d('widget_open'),
                    !(
                        s() <= 0 ||
                        (y() &&
                            ((g = c()),
                            !t ||
                                'function' !=
                                    typeof t.doesClinicalSessionMatchRoute ||
                                !0 !== t.doesClinicalSessionMatchRoute(g)))
                    ))
                )
                    return (
                        r(),
                        void (
                            y() &&
                            Promise.resolve(u({ render: !0 })).catch(() => {})
                        )
                    );
                var g;
                y()
                    ? Promise.resolve(u({ render: !0 }))
                          .then((t) => {
                              if (s() > 0) return (r(), void l());
                              const e = t && 'object' == typeof t ? t : c();
                              f(v(b(), e));
                          })
                          .catch(() => {
                              w();
                          })
                    : w();
            },
            minimizeChatbot: function () {
                const t = document.getElementById('chatbotContainer');
                (t && t.classList.remove('active'), a(!1), p());
            },
            handleChatKeypress: function (t) {
                t && 'Enter' === t.key && C();
            },
            sendChatMessage: C,
            sendQuickMessage: function (e) {
                if ((d('quick_message'), 'appointment' === e))
                    return (
                        Promise.resolve(g('Quiero agendar una cita')).catch(
                            () => {}
                        ),
                        t &&
                            'function' == typeof t.startChatBooking &&
                            t.startChatBooking(),
                        void p()
                    );
                const o = n[e] || e;
                (Promise.resolve(g(o)).catch(() => {}),
                    Promise.resolve(m(o)).catch(() => {}));
            },
            scheduleInitialNotification: function (t) {
                const e = Number(t),
                    o = Number.isFinite(e) && e >= 0 ? e : 8e3;
                setTimeout(() => {
                    const t = document.getElementById('chatNotification'),
                        e = i(),
                        o = s();
                    e || 0 !== o || (t && (t.style.display = 'flex'), P());
                }, o);
            },
            showTeaser: P,
            hideTeaser: k,
        }));
})();
