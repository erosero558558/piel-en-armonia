...
function it(t, n, e) {
    ...
    const s = i.querySelector(n);
    s instanceof HTMLElement && s.setAttribute(e, 'true');
}
const st = 'adminLastSection',
    ot = 'adminSidebarCollapsed';
function lt(a, { persist: t = !1 } = {}) {
    const n = k(a);
...
function ct() {
    const a = B();
    (Fa(st, a.ui.activeSection), Fa(ot, a.ui.sidebarCollapsed ? '1' : '0'));
}
function rt() {
    const a = F();
...
function mt() {
    (y((a) => ({ ...a, ui: { ...a.ui, sidebarOpen: !1 } })), vt(), Ka());
}
function vt() {
    const t = B(),
...
async function yt(a) {
    switch (a) {
        case 'appointments_pending_transfer':
            (await ut('appointments'), w('pending_transfer'), q(''));
            break;
        case 'appointments_all':
            (await ut('appointments'), w('all'), q(''));
            break;
        case 'appointments_no_show':
            (await ut('appointments'), w('no_show'), q(''));
            break;
        case 'callbacks_pending':
            (await ut('callbacks'), L('pending'));
            break;
        case 'callbacks_contacted':
            (await ut('callbacks'), L('contacted'));
            break;
        case 'callbacks_sla_urgent':
            (await ut('callbacks'), L('sla_urgent'));
            break;
        case 'queue_sla_risk':
            (await ut('queue'), v('sla_risk'));
            break;
        case 'queue_waiting':
            (await ut('queue'), v('waiting'));
            break;
        case 'queue_called':
            (await ut('queue'), v('called'));
            break;
        case 'queue_no_show':
            (await ut('queue'), v('no_show'));
            break;
        case 'queue_all':
            (await ut('queue'), v('all'));
            break;
        case 'queue_call_next':
            (await ut('queue'), await ea(B().queue.stationConsultorio));
    }
}
...
async function St(a) {
    ...
    try {
        ...
        (Xa({
            ...
            tt(B().auth.requires2FA ? '2fa' : 'password'),
...
async function wt() {
    (!(function () {
        ...
        (function () {
            const t = a('#adminMainContent');
            (t instanceof HTMLElement &&
                t.setAttribute('data-admin-frame', 'sony_v3'),
                Object.entries(et).forEach(([a, t]) => {
                    (it(a, t.hero, 'data-admin-section-hero'),
                        it(a, t.priority, 'data-admin-priority-rail'),
                        it(a, t.workbench, 'data-admin-workbench'),
                        it(a, t.detail, 'data-admin-detail-rail'));
                }));
        })(),
                        ...
                        await (async function (a, t) {
                            switch (a) {
                                ...
                                case 'context-open-dashboard':
                                    return void (await ut('dashboard'));
                            }
                        })(n, t);
                    } catch (a) {
        ...
        (function () {
            ...
            (t?.addEventListener('click', () => {
                window.matchMedia('(max-width: 1024px)').matches ? bt() : pt();
            }),
                ...
                window.addEventListener('storage', (a) => {
                    'themeMode' === a.key && lt(String(a.newValue || 'system'));
                }));
        })(),
        window.addEventListener('beforeunload', (a) => {
            _() && (a.preventDefault(), (a.returnValue = ''));
        }));
    const n = document.getElementById('loginForm');
    (n instanceof HTMLFormElement && n.addEventListener('submit', St),
        u({
            navigateToSection: ut,
            focusQuickCommand: gt,
            focusCurrentSearch: ht,
            runQuickAction: yt,
            closeSidebar: mt,
            toggleMenu: () => {
                window.matchMedia('(max-width: 1024px)').matches ? bt() : pt();
        ...
        (await p())
            ? await (async function () {
                  (Ga(), Ka(), await ft(!1), Za(B().ui.activeSection));
              })()
            : (Ua(), Ka(), dt()),
        b(),
        window.setInterval(() => {
...
const Ct = (
    ...
    throw (console.error('admin-v3 boot failed', a), a);
});
