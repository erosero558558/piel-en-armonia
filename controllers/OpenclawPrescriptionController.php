...
class OpenclawPrescriptionController
{
    public static function savePrescription(array $context): void
        {
            ...
            $result = OpenclawController::mutateStore(static function (array $store) use ($service, $caseId, $medications, $rxId, $doctorProfile): array {
                ...
                $incomingItems = OpenclawController::normalizePrescriptionItemsPayload($medications);
    ...
    public static function checkInteractions(array $context): void
        {
            ...
            $proposed = OpenclawController::normalizeMedicationNameList($payload['proposed_medications'] ?? []);
    
            if ($caseId === '' || $proposed === []) {
                json_response(['ok' => false, 'error' => 'case_id y proposed_medications requeridos'], 400);
            }
    
            $active = OpenclawController::normalizeMedicationNameList($payload['active_medications'] ?? []);
            if ($active === []) {
                $active = OpenclawController::resolveActiveMedicationsForCase($caseId);
            }
            ...
            foreach ($db['pairs'] ?? [] as $pair) {
                $pairA = OpenclawController::normalizeMedicationKey((string) ($pair['drug_a'] ?? ''));
                $pairB = OpenclawController::normalizeMedicationKey((string) ($pair['drug_b'] ?? ''));
                if ($pairA === '' || $pairB === '') {
                    continue;
                }
    
                foreach ($proposed as $proposedMedication) {
                    $proposedKey = OpenclawController::normalizeMedicationKey($proposedMedication);
                    if ($proposedKey === '') {
                        continue;
                    }
    
                    $proposedMatchesA = OpenclawController::medicationMatchesInteraction($proposedKey, $pairA);
                    $proposedMatchesB = OpenclawController::medicationMatchesInteraction($proposedKey, $pairB);
                    if (!$proposedMatchesA && !$proposedMatchesB) {
                        continue;
                    }
    
                    foreach ($active as $activeMedication) {
                        $activeKey = OpenclawController::normalizeMedicationKey($activeMedication);
                        if ($activeKey === '') {
                            continue;
                        }
    
                        $activeMatchesA = OpenclawController::medicationMatchesInteraction($activeKey, $pairA);
                        $activeMatchesB = OpenclawController::medicationMatchesInteraction($activeKey, $pairB);
