...
class PaymentController
{
    ...
    public static function webhook(array $context): void
    {
        ...
        if ($eventType === 'checkout.session.completed') {
            ...
            self::handleSoftwareSubscriptionCheckoutCompleted($sessionData);
        ...
        if ($eventType === 'invoice.paid') {
            ...
            self::handleSoftwareSubscriptionInvoicePaid($invoiceData);
        }

        if ($eventType === 'invoice.payment_failed') {
            $invoiceData = isset($event['data']['object']) && is_array($event['data']['object']) ? $event['data']['object'] : [];
            self::handleSoftwareSubscriptionInvoiceFailed($invoiceData);
        }

        if ($eventType === 'customer.subscription.deleted') {
            $subscriptionData = isset($event['data']['object']) && is_array($event['data']['object']) ? $event['data']['object'] : [];
            self::handleSoftwareSubscriptionCanceled($subscriptionData);
        }
